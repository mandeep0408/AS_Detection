...
model1 = tf.keras.models.load_model('Smodel.h5')  # Sarcasm detection model
model2 = tf.keras.models.load_model('Hmodel.h5')  # Abusive content detection model
...
@app.route("/abusiveContentDetection", methods=['GET', 'POST'])
    ...
    if 'user_id' in session:
        ...
        if user:
            if request.method == 'POST':
                ...
                prediction = predict_abusive(input_sentence)  # Use model 2
            ...
            return render_template("abusiveContentDetection.html", user=user, input_sentence=input_sentence, prediction=prediction)
        else:
...
def sarcasmContentDetection():
    ...
    if 'user_id' in session:
        ...
        if user and request.method == 'POST':
            input_sentence = request.form['text']
            prediction = predict_sarcasm(input_sentence)  # Use model 1
            binary_prediction = 1 if prediction > 0.5 else 0
...
# Define the predict_abusive function
...
# Define the predict_sarcasm function
